...
// Redux
import { rateDay, setMood, setMoodIntensity, addActivity, setActivities } from '../../../redux/slices';
import { DayPart as DayPartState, DaysTrackerState, Day, Days } from '../../../redux/slices';
...
type BaseDailyInputProps = {
    ...
    rateDay: ActionCreatorWithPayload<{ mood: string, moodIntensity: string; activities: Array<string>; }, string>;
...
const BaseDailyInput: FC<BaseDailyInputProps> = (props) => {
    const { mood, moodIntensity, activities, localDays, serverDays, rateDay, setMood, setMoodIntensity, addActivity, setActivities } = props;

    // TODO
    // Replace myActivities with redux activities
    const [activity, setActivity] = useState<string>('');
    const [myActivities, setMyActivities] = useState<Array<Activity>>([new Activity('Run'), new Activity('Have fun'), new Activity('Sleep'), new Activity('Create'), new Activity('Victoria time!'), new Activity('Persuade them')]);
    ...
    const [myIntensityWarnings, setMyIntensityWarnings] = useState<Set<string>>(new Set());
    ...
    const keyExtractor = useCallback((item, index) => `my-activity-${item.id}`, []);
...
const List: FC<ListProps> = (props) => {
    const { mood, moodIntensity, data, keyExtractor, onDragEnd, isFocused, onScrollUp, onScrollDown } = props;
...
const mapStateToProps = (state: { dayPart: DayPartState, days: DaysTrackerState }) => {
...
const mapDispatch = {
    rateDay, setMood, setMoodIntensity, addActivity, setActivities
};
